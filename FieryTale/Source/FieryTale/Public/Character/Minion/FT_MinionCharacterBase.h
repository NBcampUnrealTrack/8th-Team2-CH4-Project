...
public:
	...
	// [IAbilitySystemInterface 순정 오버라이드] 미니언 자체 내장형 ASC 주소지를 사출합니다.
	virtual UAbilitySystemComponent* GetAbilitySystemComponent() const override { return AbilitySystemComponent; }

protected:
	/** 월드 진입 즉시 서버 주권 하에 스탯 수치와 피아식별 라인을 동기화 낙인찍는 관문 */
	virtual void BeginPlay() override;

	/** 미니언 본체에 직통 내장되어 쿨다운과 격발을 통제할 고유 GAS 컴포넌트 */
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "FieryTale | Minion | GAS")
	TObjectPtr<UAbilitySystemComponent> AbilitySystemComponent;

	/** 기획자가 월드 배치 창(디테일 패널)이나 스폰 매니저에서 지정할 이 미니언의 AOS 팀 세력 태그 
	 * (예: FTTags::FTFaction::Team_Blue 혹은 Team_Red)
	 */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "FieryTale | Minion | Design")
	FGameplayTag MinionTeamTag;

	/** 앞서 빌드한 데이터 에셋(비주얼 메쉬, 체력, 이동 속도, 공격력, 스킬 셋)을 주입받는 통제 슬롯 */
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "FieryTale | Minion | Design")
	TObjectPtr<UFT_MinionData> MinionData;

private:
	/** 주입된 데이터 에셋과 세력 태그를 서버 주권 하에 코어 장부에 영구 낙인하는 인프라 함수 */
