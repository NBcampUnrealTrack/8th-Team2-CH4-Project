...
public:
    ...
    // --- 🎯 AttributeSet 초기화용 탄퍼짐 원본 데이터 (카테고리 싱크 완료) ---
    /** 이 무기가 가질 태생적인 기본 탄퍼짐 각도 수치 (어트리뷰트셋 초기화 기준값) */
    UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "FieryTale|Spread Data")
    float InitBaseSpread;

    /** 이 무기가 난사 등으로 인해 최대치로 벌어질 수 있는 탄퍼짐 한계 수치 (어트리뷰트셋 초기화 한계값) */
    UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "FieryTale|Spread Data")
    float InitMaxSpread;

    /** Projectile 타입 영웅일 때 발사할 투사체 액터 클래스 (투사체 타입일 때만 활성화) */
