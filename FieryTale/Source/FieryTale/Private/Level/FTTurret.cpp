...
AFTTurret::AFTTurret()
{
    ...
    AbilitySystemComponent->SetReplicationMode(EGameplayEffectReplicationMode::Minimal); // 포탑은 AI 판정이므로 Minimal 모드 사용
...
void AFTTurret::SetGenericTeamId(const FGenericTeamId& InTeamID)
{
    TeamId = InTeamID; // 외부에서 강제로 팀 ID를 주입할 때 업데이트
}

FGenericTeamId AFTTurret::GetGenericTeamId() const
{
    return TeamId; // 피아식별 연산 시 현재 캐싱된 팀 ID 반환
}
...
void AFTTurret::BeginPlay()
{
    ...
    if (AbilitySystemComponent) // ASC 유효성 검증
    {
        ...
        TeamId = FGenericTeamId(static_cast<uint8>(TurretTeam)); // 인공지능 피아식별용 ID 할당

        AttributeSet->InitMaxHealth(1000.0f); // GAS 표준화 초기 최대 체력 1000 주입
        AttributeSet->InitHealth(1000.0f); // GAS 표준화 초기 현재 체력 1000 주입
