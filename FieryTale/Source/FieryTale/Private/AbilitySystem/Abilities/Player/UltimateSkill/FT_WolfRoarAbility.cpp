...
{
    // 캐릭터마다 고유의 궁극기 스펙과 런타임 타깃 장부를 격리 관리하기 위해 액터당 인스턴스화 모드로 관리합니다.
    InstancingPolicy = EGameplayAbilityInstancingPolicy::InstancedPerActor;
    
    // 키 입력 즉시 로컬 화면에서 하울링 시각 효과가 켜지도록 클라이언트 선입력 예측 시스템을 구동합니다.
    NetExecutionPolicy = EGameplayAbilityNetExecutionPolicy::LocalPredicted;

    // 궁극기 범주 분류 및 연계 시스템 식별을 위해 전용 에셋 태그를 완착합니다.
...
{
    // 마스터 베이스 클래스의 게이지 비용 소모 파이프라인을 가동하여 비용 소모 및 검증을 최상단 관문에서 처리합니다.
    ...
    // 다중 타깃 오버랩 스캔 및 타인의 ASC 장부에 원격으로 대미지, 속박 GE를 주입하는 민감한 연산은 주권 권한을 지닌 데디케이트 서버 내부에서만 가동되도록 완벽히 가둡니다.
    ...
    if (K2_HasAuthority())
    {
        ...
        // 부채꼴 리치 반경에 맞추어 사거리 내 원형 구체 형태의 1차 충돌 풀을 휩쓸기 스캔합니다.
            ...
            for (const FOverlapResult& Result : OverlapResults)
            {
                ...
                if (TargetASC)
                {
                    ...
                    if (DamageGameplayEffectClass)
                    {
                        ...
                        {
                            // 대미지 수신 태그 장부 상수를 밀어 넣어 GEEC_Damage 수선본과 완벽히 동기화합니다.
